...
class TextInference:
    ...
    def process_text(self, prompt):
        # Use the existing create_completion method, but collect all chunks into a single string
        full_response = ""
        for chunk in self.inference.create_completion(prompt, stream=True):
            full_response += chunk["choices"][0]["text"]
        return full_response

    def extract_candidate_info(self, transcript):
        prompt = f"""
        Extract the following information from the given interview transcript and format it as JSON:
        - Name
        - School
        - Degree
        - Graduation Year
        ...
        full_response = ""
        completion = self.inference.create_completion(prompt, stream=False)
        full_response = completion['choices'][0]['text']
        full_response = full_response.split('```')[1][6:]
        try:
